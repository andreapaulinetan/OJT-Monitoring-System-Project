...
public class UpdateSubmissionServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) 
        ...
        if (user == null || !"admin".equalsIgnoreCase(user.getRole())) {
            util.ErrorLogger.logError("SECURITY VIOLATION", "Unauthorized attempt to update submission. Active user: " + (user != null ? user.getEmail() : "anonymous") + " (Tab ID: " + tabId + ")", null, session, getServletContext());
        ...
        try {
            ...
            if (isUpdated) {
                ...
                util.ErrorLogger.logError("DATABASE TRANSACTION ERROR", "Failed to update submission in database for ID: " + submissionId, null, request.getSession(false), getServletContext());
                response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Failed to update submission.");
            }
            
        } catch (Exception e) {
            util.ErrorLogger.logError("SERVLET UPDATE ERROR", "Failed to update submission due to input or processing exception. Submission ID: " + request.getParameter("submissionId"), e, request.getSession(false), getServletContext());
            e.printStackTrace();
            response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Failed to update submission.");
        }
    }
